...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#EBEBEB] font-sans flex flex-col">
      <nav className="bg-white border-b border-[#A5CEE0] px-8 py-4 sticky top-0 z-30 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#10367D] rounded-xl flex items-center justify-center shadow-md text-white font-black text-xs">EQ</div>
          <h1 className="text-xl font-black text-[#10367D] uppercase tracking-tight">EARIST Kiosk</h1>
        </div>
        {isAuthenticated && (
          <Button variant="ghost" onClick={() => signOut()} className="text-slate-400 hover:text-[#10367D] uppercase text-[10px] font-black tracking-widest">
      ...
      <main className="max-w-7xl mx-auto w-full px-8 py-12 flex flex-col lg:flex-row gap-10">
        ...
        <div className="lg:w-1/3">
          <Card className="border-0 shadow-2xl rounded-[48px] overflow-hidden bg-white sticky top-24">
            <div className="bg-[#10367D] p-10 text-center text-white">
              <h2 className="text-3xl font-black uppercase leading-none tracking-tighter">Get Started</h2>
              <p className="text-[#EBEBEB] mt-3 text-sm font-medium">Scan Student ID to book</p>
            </div>
            <CardContent className="p-10 text-center">
              <div className="mb-10 p-8 rounded-[32px] bg-[#A5CEE0]/30 border-2 border-dashed border-[#A5CEE0] flex flex-col items-center">
                 <QrCode className="w-16 h-16 text-[#10367D] mb-4" />
                 <h3 className="font-black text-slate-800 uppercase tracking-[0.2em] text-[10px]">Scanner Active</h3>
              </div>
              <Button onClick={() => setLocation("/kiosk")} className="w-full bg-[#10367D] hover:bg-[#10367D] h-20 rounded-[32px] font-black uppercase text-white shadow-xl shadow-[#10367D]/20 text-lg transition-transform active:scale-95">
                Start Booking
              </Button>
            </CardContent>
          </Card>
        ...
        <div className="lg:w-2/3 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <Card className="border-0 shadow-xl rounded-[48px] bg-white overflow-hidden border-t-8 border-[#10367D] flex flex-col">
            <CardContent className="p-10 flex-1 flex flex-col">
              <div className="flex justify-between items-center mb-8">
                <h3 className="font-black text-slate-800 uppercase text-xl flex items-center gap-3 tracking-tighter">
                  <Monitor size={24} className="text-[#10367D]"/> Live Monitor
                </h3>
                ...
                <div className="relative">
                  <button 
                    ...
                    className="bg-slate-50 border border-slate-100 px-4 py-2.5 rounded-2xl flex items-center gap-2 text-[10px] font-black text-slate-600 uppercase transition-all hover:border-[#10367D]"
                  >
                    ...
                    <div className="absolute top-full right-0 mt-2 w-64 bg-white shadow-2xl rounded-3xl overflow-hidden z-50 border border-slate-50 animate-in fade-in slide-in-from-top-2">
                      {faculties.map(f => (
                        <button 
                          ...
                          className="w-full text-left px-5 py-4 hover:bg-[#A5CEE0]/30 text-xs font-black text-slate-700 border-b border-slate-50 last:border-0 uppercase transition-colors"
                        >
                          {f.name}
                        </button>
                      ))}
              ...
              <div className="bg-slate-50 rounded-[40px] p-8 text-center border border-slate-100 mb-8 shadow-inner">
                <Badge className="bg-[#A5CEE0]/30 text-[#10367D] border-0 text-[10px] font-black mb-3 uppercase tracking-widest px-4 py-1.5 rounded-full">
                   Now Consulting
                </Badge>
                ...
                {currentServing && (
                  <p className="text-[10px] font-black text-[#10367D] mt-6 uppercase tracking-widest flex items-center justify-center gap-2">
              ...
              <div className="space-y-3 mb-10 flex-grow">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2 mb-2">Upcoming Tickets:</p>
                {upNextQueue.length > 0 ? upNextQueue.slice(0, 3).map((s, idx) => (
                  <div key={s.id} className="bg-white p-5 rounded-2xl border border-slate-100 flex justify-between items-center shadow-sm hover:border-[#10367D] transition-colors">
                    <span className="text-sm font-black text-slate-700 tracking-tight">{s.student_display_name}</span>
                    <Badge className="bg-[#A5CEE0]/30 text-[#10367D] border-0 text-[9px] font-black px-3 py-1 uppercase tracking-tighter">
                      +{ (idx + 1) * 15 }m wait
                    </Badge>
                  </div>
                )) : (
              ...
              <form onSubmit={handleQuickSearch} className="pt-6 border-t border-slate-100 mt-auto flex items-center gap-3">
                <div className="relative flex-1">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border-transparent focus:ring-2 focus:ring-[#A5CEE0] focus:bg-white rounded-[20px] py-4 pl-12 text-[10px] font-black uppercase tracking-widest transition-all" 
                    value={statusInput} 
                    onChange={(e) => setStatusInput(e.target.value)} 
                  />
                </div>
                <button type="submit" className="bg-[#10367D] text-white p-4 rounded-[20px] shadow-lg transition-all hover:bg-[#10367D] active:scale-95 flex items-center justify-center">
                  <ArrowRight size={18} />
                </button>
              </form>
            </CardContent>
          ...
          <Card className="border-0 shadow-xl rounded-[48px] bg-white overflow-hidden flex flex-col">
            <CardContent className="p-10 flex-1 flex flex-col">
              <div className="flex items-center gap-5 mb-10">
                <div className="w-16 h-16 bg-[#A5CEE0]/30 rounded-[28px] flex items-center justify-center text-[#10367D] shadow-inner">
                  <Calendar size={32} />
                </div>
                <div>
                  <h3 className="font-black text-slate-800 uppercase text-2xl leading-none tracking-tighter">Schedule</h3>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Available Hours</p>
                </div>
              </div>

              <div className="space-y-6 flex-1">
                <div className="p-8 bg-slate-50 rounded-[40px] border border-slate-100 transition-all hover:bg-white hover:shadow-md">
                  <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-4">Consultation Windows</p>
                ...
                <div className="p-8 bg-slate-50 rounded-[40px] border border-slate-100 flex flex-col items-center text-center transition-all hover:bg-white hover:shadow-md">
                  <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-4">Meeting Preference</p>
                  <div className="flex items-center gap-3 bg-white px-8 py-4 rounded-[24px] shadow-sm border border-[#A5CEE0]">
                    {isOnline ? <Globe size={24} className="text-[#10367D]" /> : <UserCheck size={24} className="text-[#10367D]" />}
              ...
              <button 
                ...
                className="mt-12 text-[10px] font-black text-slate-300 hover:text-[#10367D] uppercase tracking-[0.4em] text-center w-full transition-colors border-t border-slate-50 pt-8"
              >
