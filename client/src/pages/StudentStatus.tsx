...
export default function StudentStatus() {
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#EBEBEB]">
      <Loader2 className="w-12 h-12 animate-spin text-[#10367D] mb-4" />
      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Syncing Live Status...</p>
    </div>
  );

  if (error || !queueEntry || !faculty) {
    return (
      <div className="min-h-screen bg-[#EBEBEB] p-8 flex flex-col items-center justify-center text-center">
        <div className="w-20 h-20 bg-[#A5CEE0]/30 text-[#10367D] rounded-[28px] flex items-center justify-center mb-6">
           <AlertCircle size={40} />
        </div>
        <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter mb-2">Ticket Not Found</h2>
        <p className="text-slate-400 font-medium max-w-xs mb-8">This session may have ended or the Ticket ID entered is incorrect.</p>
        <Button onClick={() => setLocation("/kiosk")} className="bg-[#10367D] hover:bg-[#10367D] text-white rounded-2xl h-16 px-10 font-black uppercase tracking-widest shadow-xl shadow-[#10367D]/20 active:scale-95 transition-all">
          Back to Kiosk
        </Button>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#EBEBEB] flex flex-col font-sans">
      <header className="px-8 py-6 flex items-center justify-between bg-white border-b border-[#A5CEE0] shadow-sm sticky top-0 z-30">
        <button onClick={() => setLocation("/kiosk")} className="text-slate-400 hover:text-[#10367D] transition-colors flex items-center gap-2 font-black text-[10px] uppercase tracking-widest">
          <ChevronLeft className="w-5 h-5" /> Kiosk
        </button>
        <div className="text-center">
          <h1 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">TICKET: {queueId.slice(0, 8).toUpperCase()}</h1>
        </div>
        <button onClick={loadQueueData} className="text-slate-400 hover:text-[#10367D] active:rotate-180 transition-transform duration-500">
      ...
      <main className="flex-1 p-8 flex flex-col items-center max-w-2xl mx-auto w-full">
        ...
        <div className="text-center mb-10">
           <motion.div 
            ...
            className={`inline-flex items-center justify-center w-20 h-20 rounded-[32px] mb-6 shadow-lg ${isCalled ? 'bg-[#10367D] text-white' : 'bg-[#A5CEE0]/30 text-[#10367D]'}`}
           >
           ...
           <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-3">Active Session for Student {queueEntry.student_number}</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mb-8">
          {/* POSITION CARD */}
          <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="relative">
            <div className={`absolute -top-4 left-6 right-6 h-12 rounded-t-[32px] z-0 shadow-inner ${isCalled ? 'bg-[#10367D]' : 'bg-[#A5CEE0]/45'}`} />
            <Card className="relative z-10 border-0 shadow-[0_20px_40px_rgba(0,0,0,0.04)] rounded-[40px] overflow-hidden bg-white text-center p-10">
              <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em] mb-2">Queue Position</p>
              <h3 className={`text-7xl font-black tracking-tighter ${isCalled ? 'text-[#10367D]' : 'text-slate-800'}`}>
              ...
              <div className="mt-6 pt-6 border-t border-slate-50 flex items-center justify-center gap-2 text-slate-500 font-black text-[10px] uppercase tracking-widest">
                <Clock className="w-4 h-4 text-[#10367D]" /> Est. {totalWait} mins
              </div>
            </Card>
          </motion.div>
            ...
            <Card className="border-0 shadow-[0_20px_40px_rgba(0,0,0,0.04)] rounded-[40px] bg-white p-10 h-full flex flex-col justify-between">
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-[#10367D] shadow-inner"><Users className="w-6 h-6" /></div>
                  <div>
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Professor</p>
                    <p className="text-lg font-black text-slate-800 leading-tight">{faculty.name}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-[#10367D] shadow-inner"><MapPin className="w-6 h-6" /></div>
                  <div>
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Mode</p>
                    <p className="text-lg font-black text-slate-800 uppercase tracking-tighter">{queueEntry.consultation_type?.replace('_', ' ')}</p>
                  </div>
                </div>
              </div>
              <button onClick={handleShare} className="w-full mt-8 py-4 bg-slate-50 hover:bg-[#A5CEE0]/30 hover:text-[#10367D] text-slate-400 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-2 border border-slate-100 transition-all">
        ...
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="w-full">
          <Card className="border-0 shadow-[0_30px_60px_rgba(0,0,0,0.06)] rounded-[48px] bg-white p-12 text-center flex flex-col items-center gap-8 border-t-8 border-[#10367D]">
            <div className="p-6 bg-white rounded-[40px] shadow-inner border-2 border-dashed border-[#A5CEE0] group hover:border-[#10367D] transition-colors">
               <QRCodeSVG value={shareUrl} size={180} />
            </div>
            <div>
              <h3 className="font-black text-slate-900 uppercase tracking-tight text-xl">Take it with you</h3>
              <p className="text-xs text-slate-400 font-bold uppercase mt-2 tracking-widest">Scan to track from your mobile device.</p>
            </div>
            <div className="flex items-center gap-2 text-[10px] font-black text-[#10367D] uppercase tracking-[0.2em]">
              <div className="h-2 w-2 rounded-full bg-[#A5CEE0]/300 animate-pulse" /> Live Updates Active
            </div>
          </Card>
        </motion.div>
