    ...
    <div className="min-h-screen bg-[#EBEBEB] flex flex-col font-sans">
      ...
      <header className="px-10 py-8 flex justify-between items-center">
        <button 
          ...
          className={`flex items-center gap-2 font-bold uppercase text-[10px] tracking-[0.2em] transition-all ${step === "college" ? 'opacity-0' : 'text-slate-400 hover:text-[#10367D]'}`}
        >
          <ChevronLeft className="w-4 h-4" /> Back
        </button>

        <div className="text-center">
          <h1 className="text-sm font-black text-[#10367D] uppercase tracking-[0.3em]">Booking Portal</h1>
          <p className="text-slate-400 font-bold text-[10px] mt-0.5 uppercase tracking-widest">
            Student: <span className="text-slate-800">{studentName || "Verified Student"}</span>
          </p>
          <p className="text-slate-300 font-bold text-[9px] mt-1 uppercase tracking-widest">
            ID: <span className="text-slate-500">{maskStudentNumber(studentNumber)}</span>
          </p>
        </div>

        <div className="bg-white px-4 py-2 rounded-full border border-[#A5CEE0] shadow-sm flex items-center gap-2">
          <div className="h-2 w-2 rounded-full bg-[#10367D] animate-pulse" />
        ...
        <div className="max-w-4xl w-full">
          ...
          <div className="relative mt-8">
            ...
            <Card className="relative border-0 shadow-[0_30px_60px_rgba(0,0,0,0.06)] rounded-[48px] bg-white overflow-hidden z-10">
              <CardHeader className="pt-16 pb-8 px-12 text-center">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-[24px] bg-[#A5CEE0]/30 text-[#10367D] mb-6">
              ...
              <CardContent className="px-12 pb-20 pt-4">
                {error && (
                  <Alert variant="destructive" className="mb-8 bg-[#A5CEE0]/30 border-0 text-[#10367D] rounded-3xl p-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {step === "college" && colleges.map((college) => (
                    <button 
                      key={college.id} 
                      className="group flex items-center justify-between p-7 bg-white border border-slate-100 hover:border-[#A5CEE0] hover:shadow-xl hover:shadow-[#10367D]/10 rounded-[32px] transition-all duration-300 text-left"
                      onClick={() => handleCollegeSelect(college.id)}
                    >
                      <div>
                        <div className="font-black text-[#10367D] text-[10px] tracking-widest uppercase mb-1.5">{college.code}</div>
                        <div className="font-bold text-slate-800 text-lg leading-tight group-hover:text-[#10367D] transition-colors">{college.name}</div>
                      </div>
                      <div className="h-10 w-10 rounded-full bg-slate-50 group-hover:bg-[#A5CEE0]/30 flex items-center justify-center transition-colors">
                        <ChevronRight className="w-5 h-5 text-slate-300 group-hover:text-[#10367D]" />
                      </div>
                    </button>
                  ))}
                  {step === "college" && colleges.length === 0 && (
                    <div className="md:col-span-2 text-center py-24 bg-slate-50/50 rounded-[40px] border-2 border-dashed border-slate-100">
                      ...
                      <div className="text-slate-400 font-black uppercase tracking-widest text-[10px]">No colleges available</div>
                    </div>
                  )}
                  ...
                  {step === "department" && departments.map((dept) => (
                    ...
                    >
                      <div>
                        <div className="font-black text-[#10367D] text-[10px] tracking-widest uppercase mb-1.5">{dept.code}</div>
                        <div className="font-bold text-slate-800 text-lg leading-tight group-hover:text-[#10367D] transition-colors">{dept.name}</div>
                      </div>
                      <div className="h-10 w-10 rounded-full bg-slate-50 group-hover:bg-[#A5CEE0]/30 flex items-center justify-center transition-colors">
                        <ChevronRight className="w-5 h-5 text-slate-300 group-hover:text-[#10367D]" />
                      </div>
                  ...
                  {step === "department" && departments.length === 0 && (
                    <div className="md:col-span-2 text-center py-24 bg-slate-50/50 rounded-[40px] border-2 border-dashed border-slate-100">
                      ...
                      <div className="text-slate-400 font-black uppercase tracking-widest text-[10px]">No departments available</div>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-1 gap-4 md:col-span-2">
                      {faculties.length > 0 ? faculties.map((faculty) => {
                        ...
                        return (
                          <button 
                            key={faculty.id} 
                            className="group flex items-center justify-between p-6 bg-white border border-slate-100 hover:border-[#A5CEE0] hover:shadow-lg rounded-[32px] transition-all duration-300 text-left"
                            onClick={() => handleFacultySelect(faculty.id)}
                          >
                            <div className="flex items-center gap-6">
                              <div className="h-14 w-14 rounded-2xl bg-[#A5CEE0]/30 flex items-center justify-center text-[#10367D] font-black text-xl shadow-sm">
                                {faculty.name[0]}
                              </div>
                              <div>
                                <div className="font-black text-slate-800 text-xl group-hover:text-[#10367D] transition-colors">{faculty.name}</div>
                                <div className="flex items-center gap-2 mt-1">
                                  <Clock className="w-3 h-3 text-slate-300" />
                                  <span className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">
                                    Est. Wait: <span className="text-[#10367D]">{typeof wait === 'number' ? `~${wait} mins` : wait}</span>
                                  </span>
                                </div>
                              </div>
                            </div>
                            <Badge className="bg-[#A5CEE0]/30 text-[#10367D] border-0 uppercase text-[9px] px-3 py-1.5 font-black tracking-widest rounded-full">Available</Badge>
                          </button>
                        );
                      }) : (
                        <div className="text-center py-24 bg-slate-50/50 rounded-[40px] border-2 border-dashed border-slate-100">
                          ...
                          <div className="text-slate-400 font-black uppercase tracking-widest text-[10px]">No Active Professors</div>
                        </div>
                      )}
                    </div>
                  )}
                    ...
                    <div className="md:col-span-2 space-y-5">
                      <div className="rounded-[28px] border border-slate-100 bg-slate-50 p-6 text-left">
                        ...
                        <textarea
                          ...
                          className="w-full min-h-[120px] rounded-2xl border border-slate-200 bg-white p-4 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#A5CEE0]"
                          maxLength={500}
                        />
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 text-right">
                            ...
                            <button
                              ...
                              className={`rounded-2xl border p-4 text-left transition-all ${
                                selectedSlotKey === slot.key
                                  ? "border-[#10367D] bg-[#A5CEE0]/30"
                                  : "border-slate-200 bg-white hover:border-[#A5CEE0]"
                              }`}
                            >
                              ...
                              <p className="text-[10px] font-black uppercase tracking-widest text-[#10367D] mt-2">
                      ...
                      <Button
                        ...
                        className="w-full h-16 bg-[#10367D] hover:bg-[#10367D] rounded-2xl font-black text-white uppercase tracking-[0.2em]"
                      >
                        Confirm Booking
                      </Button>
                    </div>
          ...
          <div className="mt-8">
             <Button
              ...
              disabled={loading}
              variant="outline"
              className="w-full h-14 border-[#10367D] text-[#10367D] bg-white hover:bg-[#A5CEE0]/30 rounded-2xl font-black text-sm uppercase tracking-[0.2em] shadow-sm"
             >
              Cancel Booking
             </Button>
          </div>
      ...
      {loading && (
        <div className="fixed inset-0 bg-white/90 backdrop-blur-md z-50 flex flex-col items-center justify-center animate-in fade-in duration-300">
          <div className="relative">
            <Loader2 className="w-20 h-20 animate-spin text-[#10367D] opacity-20" />
            <CheckCircle2 className="w-10 h-10 text-[#10367D] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-pulse" />
          </div>
          <p className="font-black text-[#10367D] uppercase tracking-[0.4em] text-[10px] mt-8">Booking Ticket...</p>
        </div>
      )}
    </div>
  );
