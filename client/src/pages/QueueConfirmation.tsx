...
export default function QueueConfirmation() {
  ...
  if (error || !queueData) return (
    <div className="min-h-screen bg-[#f3f1f6] flex items-center justify-center p-4 sm:p-6">
      <div className="max-w-md w-full">
        <Alert variant="destructive" className="bg-[#f3f1f6]/60 border-0 text-[#c62828] rounded-[32px] p-6 sm:p-8 shadow-sm">
          <AlertCircle className="h-8 w-8 mb-4" />
          <AlertDescription className="font-bold text-lg leading-tight">{error || "Booking not found"}</AlertDescription>
        </Alert>
        <Button onClick={() => setLocation("/kiosk")} className="mt-6 w-full h-14 sm:h-16 bg-[#c62828] hover:bg-[#c62828] rounded-2xl text-base sm:text-lg font-black uppercase tracking-widest">
          Return to Kiosk
        </Button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-[#f3f1f6] flex flex-col font-sans items-center justify-center p-4 sm:p-6">
      ...
      >
        <Card className="border-0 shadow-[0_40px_80px_rgba(0,0,0,0.1)] rounded-[56px] bg-white overflow-hidden text-center relative">
          {/* Header Section */}
          <div className="bg-[#c62828] pt-12 sm:pt-14 pb-8 sm:pb-10 px-5 sm:px-8 md:px-10 text-center relative overflow-hidden">
            ...
            <CardTitle className="text-3xl sm:text-4xl font-black text-white relative z-10 tracking-tighter uppercase">Confirmed!</CardTitle>
          ...
          <CardContent className="px-5 sm:px-8 md:px-10 py-8 sm:py-10 md:py-12">
            {/* QR Code Section */}
            ...
            {/* TICKET DETAILS */}
            <div className="bg-slate-50 rounded-[32px] p-5 sm:p-8 mb-10 space-y-5 border border-slate-100/50">
              <div className="flex justify-between items-start gap-3 border-b border-slate-200/50 pb-4">
                <span className="text-[10px] font-black text-[#c62828]/65 uppercase tracking-widest text-left">Student Name</span>
                <span className="text-sm font-black text-slate-800 text-right max-w-[62%] break-words">{studentName || "Verified Student"}</span>
              </div>
              <div className="flex justify-between items-start gap-3 border-b border-slate-200/50 pb-4">
                <span className="text-[10px] font-black text-[#c62828]/65 uppercase tracking-widest text-left">Student ID</span>
                <span className="text-sm font-black text-slate-800 text-right max-w-[62%] break-words">{maskStudentNumber(queueData?.student_number || "")}</span>
              </div>
              <div className="flex justify-between items-start gap-3 border-b border-slate-200/50 pb-4">
                <span className="text-[10px] font-black text-[#c62828]/65 uppercase tracking-widest text-left">Professor</span>
                <span className="text-sm font-black text-slate-800 text-right max-w-[62%] break-words">{queueData?.faculty?.name}</span>
              </div>
        ...
        <p className="mt-8 sm:mt-10 text-center text-[10px] font-black text-[#c62828]/55 uppercase tracking-[0.28em] sm:tracking-[0.4em]">
