...
export default function QueueConfirmation() {
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#EBEBEB]">
      ...
      <p className="font-black text-[#10367D] uppercase tracking-[0.3em] text-xs">Generating Ticket...</p>
    </div>
  );
  ...
  if (error || !queueData) return (
    <div className="min-h-screen bg-[#EBEBEB] flex items-center justify-center p-6">
      <div className="max-w-md w-full">
        <Alert variant="destructive" className="bg-[#A5CEE0]/30 border-0 text-[#10367D] rounded-[32px] p-8 shadow-sm">
          <AlertCircle className="h-8 w-8 mb-4" />
          <AlertDescription className="font-bold text-lg leading-tight">{error || "Booking not found"}</AlertDescription>
        </Alert>
        <Button onClick={() => setLocation("/kiosk")} className="mt-6 w-full h-16 bg-[#10367D] hover:bg-[#10367D] rounded-2xl text-lg font-black uppercase tracking-widest">
          Return to Kiosk
        </Button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-[#EBEBEB] flex flex-col font-sans items-center justify-center p-6">
      ...
      >
        <Card className="border-0 shadow-[0_40px_80px_rgba(0,0,0,0.1)] rounded-[56px] bg-white overflow-hidden text-center relative">
          {/* Header Section */}
          <div className="bg-[#10367D] pt-14 pb-10 px-10 text-center relative overflow-hidden">
            ...
            <CardDescription className="text-[#EBEBEB] font-bold mt-2 relative z-10 text-xs uppercase tracking-widest opacity-80">
          ...
          <CardContent className="px-10 py-12">
            ...
            <div className="flex flex-col items-center mb-10">
              <div className="bg-white p-6 rounded-[40px] border-2 border-dashed border-[#A5CEE0] shadow-inner relative group transition-all hover:border-[#A5CEE0]">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-white px-4 py-0.5 rounded-full text-[10px] font-black text-[#10367D] uppercase tracking-[0.2em] border border-[#A5CEE0] flex items-center gap-2">
            ...
            <div className="bg-slate-50 rounded-[32px] p-8 mb-10 space-y-5 border border-slate-100/50">
              <div className="flex justify-between items-center border-b border-slate-200/50 pb-4">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Student Name</span>
                <span className="text-sm font-black text-slate-800">{studentName || "Verified Student"}</span>
              </div>
              <div className="flex justify-between items-center border-b border-slate-200/50 pb-4">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Student ID</span>
                <span className="text-sm font-black text-slate-800">{maskStudentNumber(queueData?.student_number || "")}</span>
              </div>
              <div className="flex justify-between items-center border-b border-slate-200/50 pb-4">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Professor</span>
                <span className="text-sm font-black text-slate-800">{queueData?.faculty?.name}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Method</span>
                <Badge variant="outline" className="bg-[#A5CEE0]/30 text-[#10367D] border-[#A5CEE0] font-black px-4 py-1.5 rounded-xl text-[10px] uppercase">
            ...
            <div className="space-y-4">
              <Button 
                ...
                className="w-full bg-[#10367D] hover:bg-[#10367D] text-white font-black h-16 rounded-2xl shadow-xl shadow-[#10367D]/10 flex items-center justify-center gap-3 uppercase tracking-widest text-sm transition-all active:scale-95"
              >
              ...
              <div className="grid grid-cols-2 gap-4">
                <Button 
                  variant="outline" 
                  className="h-14 border-slate-100 text-slate-500 rounded-2xl font-black uppercase text-[10px] tracking-widest flex gap-2 hover:bg-slate-50 hover:text-[#10367D] transition-all"
